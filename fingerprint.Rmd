---
title: "GO terms and CORUM complexes enrichment for a given bait-interactors dataset"
...
Visualize mean number of interactors per bait:
```{r message=FALSE, warning=FALSE}
countVals = data.frame()
for (i in 1:length(geneNames)) {
  count = length(geneNames[[i]])
  countVals = rbind(countVals, data.frame(geneCount = count, bait = names(geneNames)[i], stringsAsFactors = FALSE))
}
moy = sum(countVals$geneCount)/length(geneNames)
minCount = min(countVals$geneCount)
maxCount = max(countVals$geneCount)

countVals = countVals[order(countVals$geneCount),]
levels = factor(countVals$bait, level = countVals$bait)

p = ggplot() + 
  geom_col(data = countVals, aes(x = levels, y = geneCount), fill="darkblue", width=.2) +
  geom_boxplot(data = countVals, aes(y=geneCount), position = position_identity()) +
  labs(y = "Interactors count", x="Bait", title="Interactors count per bait") +
  theme(text = element_text(size=20), axis.text.x = element_text(angle=90, hjust=1))
print(p)

# gBox

totalCount = sum(countVals$geneCount)
summary(countVals$geneCount)
```
...
Here the list of enriched terms will be cleared of redundancy between CC terms and unique terms are highlighted
```{r include=FALSE}
...
Redundant terms occurrences are counted and related baits are stored to generate PDF and vizualize most redundant terms
```{r include=FALSE}
nonUniqueBPTerms = enrichedGOdata[which(enrichedGOdata$unique == FALSE & enrichedGOdata$ont == "BP"), "goID"]
nonUniqueCCTerms = enrichedGOdata[which(enrichedGOdata$unique == FALSE & enrichedGOdata$ont == "CC"), "goID"]

nonUniqueBPTermsCount = as.data.frame(table(nonUniqueBPTerms), stringsAsFactors = FALSE)
nonUniqueBPTermsCount$term = Term(nonUniqueBPTermsCount$nonUniqueBPTerms)
for (i in 1:nrow(nonUniqueBPTermsCount)) {
  goID = nonUniqueBPTermsCount[i, "nonUniqueBPTerms"]
  foundTerms = enrichedGOdata[which(enrichedGOdata$goID == goID), "bait"]
  nonUniqueBPTermsCount[i, "inBaits"] = paste(foundTerms, collapse = ";")
}

nonUniqueCCTermsCount = as.data.frame(table(nonUniqueCCTerms), stringsAsFactors = FALSE)
nonUniqueCCTermsCount$term = Term(nonUniqueCCTermsCount$nonUniqueCCTerms)
for (i in 1:nrow(nonUniqueCCTermsCount)) {
  goID = nonUniqueCCTermsCount[i, "nonUniqueCCTerms"]
  foundTerms = enrichedGOdata[which(enrichedGOdata$goID == goID), "bait"]
  nonUniqueCCTermsCount[i, "inBaits"] = paste(foundTerms, collapse = ";")
}
```
Generate PDF files:
```{r eval=FALSE, include=FALSE}
pdf("redundantGOterms.pdf", width = 15, height = 30)
grid.table(nonUniqueBPTermsCount[order(-nonUniqueBPTermsCount$Freq),])
dev.off()

pdf("redundantCCterms.pdf", width = 15, height = 40)
grid.table(nonUniqueCCTermsCount[order(-nonUniqueCCTermsCount$Freq),])
dev.off()
```
