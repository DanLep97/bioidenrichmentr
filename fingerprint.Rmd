...
filterGenes = function(GOids, ont, bait, geneRange = unlist(geneNames, use.names = FALSE) ) {
  ...
  if(GOidsLength == ancestorsLength) { # we make sure every terms has a list of ancestors
    for(i in 1:ancestorsLength) {
      ...
      genesOfTerm = gene2GOdf %>%
        filter(uniprotID %in% geneRange & goID == term)
...
#for(i in 1:2) {
  BPterms = enrichGOterms("M", "BP") # enrich biological processes
  CCterms = enrichGOterms("M", "CC") # enrich cellular locations
...
#noDuplicatedEnrichedGenes = enrichedGenes[!duplicated(enrichedGenes[c("uniprotID")]),] deprecated behavior
